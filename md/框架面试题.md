 ...
 * 缺点：
 * 	深度监听，需要递归到底，一次性计算量大
 *	对data新增属性或删除 不会触发视图更新 （Vue.set Vue.delete）
	1、深度监听，需要递归到底，一次性计算量大
	2、新增属性、删除属性，监听不到
	3、无法监听数组，需要改写数组原生方法，然后改变数组的原型
 *
 * 监听数组 vue能用这些方法可以响应式是因为 相当于现在这样重新修改了数组的这些方法
 * 
 * 如何实现响应式原理的
 * 基本数据类型 对象 数组 分别是怎么处理的
 */
...
function reWriteArrayProperty() {
  ...
  // 创建新的对象 原型指向oldArrayProperty 再拓展新的方法不会影响原型
...
![image-20220731160242924](images/image-20220731160242924.png)



### diff

![image-20220731170236051](images/image-20220731170236051.png)

h函数经过一些处理，最后返回一个vnode函数（函数接收sel、data、children、text、undefined），然后这个vnode函数返回的是一个对象（return { sel, data, children, text, elm, key }; // ele是vnode对用的dom元素）

[(2条消息) vue核心面试题：diff算法_王三六的博客-CSDN博客_diff算法面试题](https://blog.csdn.net/qq_42072086/article/details/107965196)



### 初次渲染过程

![image-20220731203006099](images/image-20220731203006099.png)

执行render函数会执行getter

![image-20220731205155584](images/image-20220731205155584.png)

![image-20220731210058096](images/image-20220731210058096.png)
...
**MVC****架构就是说：M是模型、V是视图、C就是控制器处理业务的，然后他是单向数据流的，可以使得数据视图层分离**

**MVP****就是把MVC中的C变成了P（Presenter），与MVC思想基本是相通的，都是通过Controller/Presenter 来处理业务逻辑的，区别就是 MVP中的View层和Model层是完全分离的。**

**MVVM****就是Model-View-ViewModel的简写，他是MVC的改进版，与MVP基本相同，就是把P换成了VM，区别就是MVVM采用的是双向数据绑定，View更新会通知到ViewModel这样。**
...
[(2条消息) uniapp基础掌握及面试题整理(2022持续更新...)_老古懂的博客-CSDN博客_uniapp面试](https://blog.csdn.net/qq_45659769/article/details/119515064)
